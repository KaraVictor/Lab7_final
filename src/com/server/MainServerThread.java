...
//        String password = "uj3qJgrQ2RxoFWu6";
        ...
        Exchanger<ClientMessage> exchanger = new Exchanger<ClientMessage>();
        // чтение запросов
        new Thread(new ThreadReceiver(selector, serverReceiver, serverSender, connection, collection, statement, users, serverFile)).start();
...
class ThreadReceiver implements Runnable {
    ...
    ThreadReceiver(Selector selector, ServerReceiver serverReceiver, ServerSender serverSender,
                   Connection connection, Collection collection,
                   Statement statement, TreeMap<String, String> users, File serverFile) {
            ...
            while (iterator.hasNext()) {
                ...
                if(key.isReadable()) {
                    try {
                            ...
                            clientMessage = (ClientMessage) Serializer.Deserialization(byteBuffer);
                            System.out.println("запрос получен");
                            new Thread(new ThreadTreatment(connection, collection, statement, users, serverFile, clientMessage, serverSender)).start();
...
class ThreadTreatment implements Runnable {
    Connection connection;
    Collection collection;
    Statement statement;
    TreeMap<String, String> users;
    ...
    ThreadTreatment(Connection connection, Collection collection, Statement statement, TreeMap<String, String> users,
    File serverFile, ClientMessage clientMessage, ServerSender serverSender) {
        this.connection = connection;
        this.collection = collection;
        this.statement = statement;
        this.users = users;
    ...
    public synchronized void run(){
            try {
                ...
                new Thread(new ThreadSender(collection, serverSender, serverAnswer)).start();
            }
...
class ThreadSender implements Runnable {
    Collection collection;
    ServerSender serverSender;
    ServerMessage serverAnswer;

    ThreadSender (Collection collection, ServerSender serverSender, ServerMessage serverMessage) {
        this.collection = collection;
        this.serverSender = serverSender;
        this.serverAnswer = serverMessage;
    }
